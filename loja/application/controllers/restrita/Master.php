...
class Master extends CI_Controller {
    ...
    public function core($categoria_pai_id = NULL) {
        ...
        if (!$categoria_pai_id) {       
            ...
            if ($this->form_validation->run()) {
                ...
                $this->core_model->insert('categoria_pai', $data);
    ...
    public function delete($categoria_pai_id = NULL) {
        ...
        if (!$categoria_pai_id || !$this->core_model->get_by_id('categoria_pai', array('categoria_pai_id' => $categoria_pai_id))) {
            $this->session->set_flashdata('erro', 'Essa categoria pai não foi encontrada');
            redirect('restrita/master');
        }

        if ($this->core_model->get_by_id('categoria_pai', array('categoria_pai_id' => $categoria_pai_id, 'categoria_pai_ativa' => 1))) {
